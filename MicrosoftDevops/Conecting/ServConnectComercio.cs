...
namespace PipelinesTeste2.MicrosoftDevops.Conecting
{
    public class ServConnectComercio : IConnect
    {
        ...
        private readonly string _urlBase = $"{LoggedUsers.BaseUrl}/Comércio";
        ...
        public void Connect(Guid userId)
        {
            ...
            {
                throw new UnauthorizedAccessException("Erro de autenticação! As credenciais utilizadas não tem permissão para o DevOps do sistema Comércio.");
            }
            catch (Exception ex)
            {
